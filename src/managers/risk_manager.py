...
class RiskManager:
    ...
    def check_unrealized_loss(self, max_loss: float = 50.0) -> Tuple[bool, float]:
        """
        Check if unrealized loss exceeds maximum.
        
        Args:
            max_loss: Maximum allowed unrealized loss in USD
        ...
        try:
            ...
            if loss > max_loss:
                logger.warning(f"Unrealized loss (${loss:.2f}) exceeds max (${max_loss:.2f})")
                return False, loss
            
            logger.debug(f"Unrealized loss: ${loss:.2f} (max: ${max_loss:.2f})")
            return True, loss
    ...
    def check_open_positions_count(self, max_positions: int = 20) -> Tuple[bool, int]:
        """
        ...
        try:
            ...
            if count > max_positions:
                logger.warning(f"Open positions ({count}) exceed max ({max_positions})")
                return False, count
            
            logger.debug(f"Open positions: {count}/{max_positions}")
            return True, count
    ...
    def check_all_safety_conditions(self, max_loss: float = 50.0, 
                                   max_positions: int = 20) -> Tuple[bool, List[str]]:
        """
        Run all safety checks.
        
        Args:
            max_loss: Maximum allowed unrealized loss
            max_positions: Maximum allowed open positions
    ...
    def should_emergency_stop(self, max_loss: float = 50.0) -> Tuple[bool, str]:
        """
    ...
    def validate_order_placement(self, units: int, price: float, 
                                 max_margin_percent: float = 50.0) -> Tuple[bool, str]:
        """
        ...
        Args:
            ...
            price: Order price
            max_margin_percent: Maximum margin percentage to use
        ...
        try:
            ...
            max_allowed = balance * (max_margin_percent / 100)
            
            if estimated_margin > max_allowed:
                return False, f"Order would use too much margin (${estimated_margin:.2f} > {max_margin_percent}% of balance)"
    ...
    def check_market_conditions(self, spread_pips: float, 
                               max_spread: float = 2.0) -> Tuple[bool, str]:
        """
        ...
        if spread_pips > max_spread:
            logger.warning(f"Spread is {spread_pips:.1f} pips - wider than normal")
